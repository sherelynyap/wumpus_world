...
def process_percept(percept_list):
    print(percept_list)
...
def check_status(new_loc, old_loc, abs_map):
    ...
    if abs_map.world_map[new_y][new_x].wall == True:
        ...
        query = "reposition([" + abs_map.world_map[_y][agent_x].get_percepts() + "])"
        list(prolog.query(query))
        return False
...
def generate_rmap():  # Printing Agent Knowledge map...
    query_visited = [[0, 1], [0, 2], [0, 3]]  # Moved one step ahead
    x_list = []
    y_list = []
    for x in query_visited:
        if x[0] not in x_list:
            x_list.append(x[0])
        if x[1] not in y_list:
            y_list.append(x[1]);

    max_col = max(x_list) + abs(min(x_list))
    max_rows = max(y_list) + abs(min(y_list))

    print('x', x_list)
    print('y', y_list)
    r_map = R_Map(max_col + 3, max_rows + 3)
    r_map.display_r_world("Updated")
